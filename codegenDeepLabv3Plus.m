...
% 
% Run the codegen command and specify an input size of [513,513,3]. 
% This value corresponds to the input layer size of the DeepLabv3Plus network.
cfg = coder.gpuConfig('mex');
cfg.TargetLang = 'C++';
cfg.DeepLearningConfig = coder.DeepLearningConfig('cudnn');
codegen -config cfg deepLabv3Plus_predict -args {ones(513,513,3,'uint8')} -report
...
% Resize the image  to size [513,513].
sz = size(image);
img  = imresize(image, [513 513], "bilinear");
